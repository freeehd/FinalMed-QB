...
jQuery(document).ready(($) => {
  ...
  function formatPromptText(text) {
    ...
    formattedText = formattedText.replace(/\*{1,2}(.*?)\*{1,2}/g, (match, p1) => {
      if (match.startsWith('**')) {
  ...
  // --- Event Handlers ---
  appRoot.on("click", "#wqb-start-practice", () => handleStartSession("practice"))
  ...
  appRoot.on("click", "#wqb-return-to-lobby", () => (window.location.href = "/staging-area"))
  ...
  appRoot.on("change", ".wqb-specialty-checkbox", function () {
    if (!$(this).prop("checked")) {
      $(this).closest(".wqb-system").find(".wqb-system-checkbox").prop("checked", false)
    }
  })
  ...
  appRoot.on("change", "#wqb-select-all-categories", function () {
    ...
    $('.wqb-category-tree input[type="checkbox"]:not(#wqb-select-all-categories)').prop("checked", isChecked)
    updateSelectedQuestionsCounter()
  })
  ...
  function updateSelectedQuestionsCounter() {
    const checkedBoxes = $('.wqb-category-tree input[type="checkbox"]:checked:not(#wqb-select-all-categories)')
    ...
    if (window.wqbCategoryTree) {
      totalUnsolvedQuestions = calculateUnsolvedQuestions(window.wqbCategoryTree, selectedCategoryIds)
    }

    // Update the counter display
    const counterElement = $("#wqb-selected-questions-counter")
    if (counterElement.length) {
      counterElement.text(`${totalUnsolvedQuestions.toLocaleString()} unsolved questions selected`)
    }

    // Update select all checkbox state
    const allCheckboxes = $('.wqb-category-tree input[type="checkbox"]:not(#wqb-select-all-categories)')
    ...
    function processCategory(category) {
      ...
      if (selectedIds.includes(category.id)) {
        ...
        if (!hasChildren) {
          ...
          const unsolvedInCategory = category.total_questions - category.user_answered
  ...
  appRoot.on("click", ".wqb-feedback-toggle", function () {
    $(this).toggleClass("active").next(".wqb-feedback-content").slideToggle(200)
  })
...
class="wqb-session-resume-dialog">
        <div class="wqb-dialog-content">
          ...
          <p>You have an active ${sessionData.mode === "mock" ? "mock test" : "practice"} session with ${sessionData.answered_count} of ${sessionData.total_questions} questions completed.</p>
    ...
    $.ajax({
      ...
      success: (response) => {
        if (response.success) {
          ...
          $(`.wqb-active-session-item[data-session-id="${specificSessionId}"]`).fadeOut(300, function () {
            $(this).remove()
  ...
  function handleStartSession(mode = "practice") {
    sessionData = { correct: 0, attempted: 0, mode: mode, defaults: sessionData.defaults }
  ...
  function handleSubmitAnswer() {
    ...
    const questionId = selectedOption.closest(".wqb-practice-view").data("question-id")
    ...
    $.ajax({
      ...
      success: (response) => {
        if (response.success) {
          ...
          sessionData.question_states[questionId] = response.data.is_correct ? "correct" : "incorrect"
  ...
  function handleSubmitFeedback() {
    ...
    $.ajax({
      ...
      error: (xhr, status, error) => {
        console.error("AJAX Error submitting feedback:", { xhr, status, error })
        alert("A network error occurred while submitting feedback. Please try again.")
    ...
    $.ajax({
      ...
      success: (response) => {
        if (response.success) {
          ...
          sessionData.question_states = response.data.session_data.question_states || {}
  ...
  function startTimer(initialSeconds) {
    ...
    timerInterval = setInterval(() => {
      ...
      $("#wqb-timer").text(`${min.toString().padStart(2, "0")}:${sec.toString().padStart(2, "0")}`)
  ...
  function handleEndAndReview() {
    ...
    if (!sessionData.question_ids || !Array.isArray(sessionData.question_ids)) {
      console.error("sessionData.question_ids is missing or not an array:", sessionData.question_ids)
      alert("Session data is incomplete. Please try again.")
      return
    }

    if (!sessionData.user_answers || typeof sessionData.user_answers !== "object") {
      console.error("sessionData.user_answers is missing or not an object:", sessionData.user_answers)
      sessionData.user_answers = {}
    }

    if (!sessionData.question_states || typeof sessionData.question_states !== "object") {
      console.error("sessionData.question_states is missing or not an object:", sessionData.question_states)
      sessionData.question_states = {}
    }

    if (!sessionData.total || typeof sessionData.total !== "number") {
      console.error("sessionData.total is missing or not a number:", sessionData.total)
  ...
  // --- Rendering Functions ---
  function renderPracticeView({ question, current, total, is_attempted = false, user_answer = null }) {
...
//                    // <div class="wqb-progress-tracker">Correct: ${sessionData.correct} / ${sessionData.attempted}</div>
    ...
    let questionHtml = `
            <div class="wqb-practice-view" data-question-id="${question.id}">
                ...
                <div class="wqb-question-prompt">${formatPromptText(question.prompt)}</div>
    ...
    question.options.forEach((option, index) => {
      ...
      questionHtml += `<label class="wqb-option">
                                ...
                                <span class="wqb-option-text">${formatOptionText(option)}</span>
  ...
  function renderMockTestView({ question, current, total, is_attempted = false, user_answer = null }) {
    sessionData.current = current
    sessionData.total = total

    const min = Math.floor(remainingSeconds / 60)
    const sec = remainingSeconds % 60
    const currentTime = `${min.toString().padStart(2, "0")}:${sec.toString().padStart(2, "0")}`

    let questionHtml = `
            <div class="wqb-practice-view wqb-mock-test-view" data-question-id="${question.id}">
                <div class="wqb-practice-header">
  ...
  function renderNavigationButtons(is_attempted) {
    ...
    // Previous button
    if (!isFirst) {
      buttonsHtml += '<button id="wqb-prev-question" class="wqb-button-secondary">Previous</button>'
    }

    // Submit/Skip buttons
    if (!is_attempted) {
      buttonsHtml += '<button id="wqb-submit-answer" class="wqb-button-primary">Submit Answer</button>'
      if (!isLast) {
        buttonsHtml += '<button id="wqb-skip-question" class="wqb-button-secondary">Skip</button>'
      }
    } else {
      // Next button for attempted questions
      if (!isLast) {
        buttonsHtml += '<button id="wqb-next-question" class="wqb-button-primary">Next Question</button>'
      } else {
        if (sessionData.mode === "mock") {
          buttonsHtml += '<button id="wqb-finish-test-btn" class="wqb-button-primary">Finish Test</button>'
        } else {
          buttonsHtml += '<button id="wqb-finish-session-btn" class="wqb-button-primary">Finish Session</button>'
        }
      }
    }
  ...
  function renderQuestionOverview() {
    let overviewHtml = '<div class="wqb-question-overview"><div class="wqb-question-grid">'
    ...
    if (sessionData.total > maxVisibleQuestions) {
      ...
      endIndex = Math.min(sessionData.total - 1, startIndex + maxVisibleQuestions - 1)
    ...
    for (let i = startIndex; i <= endIndex; i++) {
      ...
      overviewHtml += `<button class="wqb-question-nav-btn ${statusClass}" data-question-index="${i}">${i + 1}</button>`
    }
  ...
  function updateQuestionOverview() {
    ...
    if (currentView.length) {
      currentView.find(".wqb-question-overview").replaceWith(renderQuestionOverview())
    }
  }
  ...
  function revealAnswer(data, userSelectionIndex, isReview = false) {
    ...
    if (!isReview) {
      $(".wqb-progress-tracker").text(`Correct: ${sessionData.correct} / ${sessionData.attempted}`)
    }
    ...
    const processAnalytics = (analyticsData) => {
      ...
      for (let i = 0; i < 5; i++) {
        ...
        const $percentageSpan = $(`.wqb-distribution-percentage[data-option="${i}"]`)
      ...
      $.ajax({
        ...
        success: (response) => {
          if (response.success) {
            ...
            console.error("Failed to fetch question analytics:", response.data.message)
          }
        },
        error: (xhr, status, error) => {
          console.error("AJAX Error fetching question analytics:", { xhr, status, error })
        },
      })
    }
    ...
    const optionTexts = []
    for (let i = 0; i < 5; i++) {
      const $label = $(`.wqb-option input[value="${i}"]`).closest(".wqb-option")
    ...
    const userAnswerDisplay =
      userSelectionIndex !== null
        ? `${String.fromCharCode(65 + Number.parseInt(userSelectionIndex))} – ${optionTexts[Number.parseInt(userSelectionIndex)] || ""}`
        : "Not answered"
    const correctAnswerDisplay = `${String.fromCharCode(65 + data.correct_index)} – ${optionTexts[data.correct_index] || ""}`
      ...
      <div class="wqb-question-analytics">
          ...
          <div class="wqb-analytics-grid">
              <div class="wqb-analytics-item">
                  ...
                  <span class="wqb-analytics-value ${data.is_correct ? "correct" : "incorrect"}">${userAnswerDisplay}</span>
              </div>
              <div class="wqb-analytics-item">
...
<div class="wqb-analytics-explanation">${formatExplanationHtml(data.explanation)}</div>
    ...
    if (sessionData.current_index > 0) {
      newButtonsHtml += '<button id="wqb-prev-question" class="wqb-button-secondary">Previous</button>'
    }

    if (!isLast) {
      newButtonsHtml += '<button id="wqb-next-question" class="wqb-button-primary">Next Question</button>'
    } else {
      if (sessionData.mode === "mock") {
        newButtonsHtml += '<button id="wqb-finish-test-btn" class="wqb-button-primary">Finish Test</button>'
      } else {
        newButtonsHtml += '<button id="wqb-finish-session-btn" class="wqb-button-primary">Finish Session</button>'
      }
    }
  ...
  function renderMockTestResults(data) {
    ...
    for (const specialty in data.specialty_stats) {
      ...
      const spec_percent = stat.total > 0 ? Math.round((stat.correct / stat.total) * 100) : 0
      resultsHtml += `<tr>
                                <td>${specialty}</td>
  ...
  function showSessionCompleteResults() {
    const percentage = sessionData.attempted > 0 ? Math.round((sessionData.correct / sessionData.attempted) * 100) : 0
  ...
  function renderActiveSessionDialog(activeSession, showNewSessionButton = true) {
    if (!activeSession) {
      return ""
    }
    ...
    return `
      <div class="wqb-active-session-dialog">
        ...
        <div class="wqb-session-actions">
          <button class="wqb-resume-session-btn wqb-button-primary wqb-full-width" data-session-id="${activeSession.session_id}">
            <span class="wqb-button-icon">
              <svg width="16" height="16" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                <path d="M5 12h14" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"/>
  ...
  function renderActiveSessionsList(activeSessions) {
    ...
    const sessionsHtml = activeSessions
      .map(
        (session) => `
      <div class="wqb-active-session-item" data-session-id="${session.session_id}">
        <div class="wqb-session-header">
          <div class="wqb-session-info">
            <span class="wqb-session-badge wqb-session-active">Active Session</span>
            <span class="wqb-session-mode">${session.mode === "mock" ? "Mock Test" : "Practice"}</span>
          </div>
          <button class="wqb-close-session-btn" data-session-id="${session.session_id}" title="Close Session">
            <svg width="16" height="16" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
        ...
        <div class="wqb-session-progress">
          <div class="wqb-progress-info">
            <span class="wqb-progress-text">Progress: ${session.answered_count} of ${session.total_questions} questions</span>
            <span class="wqb-progress-percentage">${session.progress_percentage}%</span>
          </div>
          <div class="wqb-progress-bar">
            <div class="wqb-progress-fill" style="width: ${session.progress_percentage}%"></div>
          </div>
        </div>
        <div class="wqb-session-details">
          <span class="wqb-session-time">Started: ${new Date(session.created_at).toLocaleDateString()}</span>
          <span class="wqb-session-expires">Expires: ${new Date(session.expires_at).toLocaleDateString()}</span>
        </div>
        <div class="wqb-session-actions">
          <button class="wqb-resume-session-btn wqb-button-primary wqb-full-width" data-session-id="${session.session_id}">
            <span class="wqb-button-icon">
              <svg width="16" height="16" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                <path d="M5 12h14" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"/>
                <path d="M12 5l7 7-7 7" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"/>
  ...
  function renderLobby() {
    ...
    $.ajax({
      ...
      success: (response) => {
        if (!response.success) {
          appRoot.html('<p class="wqb-error">Could not load the question bank. Please try again later.</p>')
          return
        }
          ...
          categories.forEach((cat) => {
            ...
            html += `<li class="wqb-category-item">
                                    <div class="wqb-category-row">
                                        ...
                                        <label>
                                            <input type="checkbox" class="${hasChildren ? "wqb-category-checkbox-parent" : ""}" name="wqb_categories[]" value="${cat.id}">
                                            ${cat.name}
                                        </label>
                                        <span class="wqb-category-progress">${cat.user_answered} of ${cat.total_questions}</span>
                                    </div>`
            if (hasChildren) {
              html += `<ul class="wqb-category-children">${buildCategoryListItems(cat.children)}</ul>`
            }
            html += `</li>`
          })
          return html
                    ...
                    <div class="wqb-lobby-redesigned">
                        ...
                        <div class="wqb-lobby-header">
                            ...
                            <div class="wqb-progress-bar-outer">
                                <div class="wqb-progress-bar-inner" style="width: ${stats.user_average_score}%;"></div>
                            </div>
                            <p class="wqb-summary-text">
                                You've answered ${stats.user_total_answered} questions with an average score of ${stats.user_average_score}%. 
...
If no categories are selected, all questions will be included in the practice session by default.                           
  ...
  function initializeLobbyHeatmap(heatmapData) {
    if (window.WQBHeatmapUtils && window.WQBHeatmapUtils.initializeLobbyHeatmap) {
      window.WQBHeatmapUtils.initializeLobbyHeatmap(heatmapData)
        .then(() => {
          ...
          if (window.WQBHeatmapUtils && window.WQBHeatmapUtils.reinitializeHeatmapOnResize) {
            window.WQBHeatmapUtils.reinitializeHeatmapOnResize("#wqb-lobby-cal-heatmap", heatmapData, {})
          }
        })
        .catch((error) => {
