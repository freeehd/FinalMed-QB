...
class Post_Types
{
    ...
    public function register()
    {
        ...
        add_action('init', [$this, 'register_taxonomy_tier']); // NEW: Hook in the new taxonomy
    ...
    /**
     * NEW: Registers the "Access Tier" taxonomy for the "Question" CPT.
     */
    public function register_taxonomy_tier()
    {
        $labels = [
            'name' => _x('Access Tiers', 'taxonomy general name', 'wqb'),
            'singular_name' => _x('Access Tier', 'taxonomy singular name', 'wqb'),
            'search_items' => __('Search Tiers', 'wqb'),
            'all_items' => __('All Tiers', 'wqb'),
            'edit_item' => __('Edit Tier', 'wqb'),
            'update_item' => __('Update Tier', 'wqb'),
            'add_new_item' => __('Add New Tier', 'wqb'),
            'new_item_name' => __('New Tier Name', 'wqb'),
            'menu_name' => __('Access Tiers', 'wqb'),
        ];

        $args = [
            'hierarchical' => false, // This makes it work like post tags
            'labels' => $labels,
            'show_ui' => true,
            'show_admin_column' => true,
            'query_var' => true,
            'rewrite' => ['slug' => 'access-tier'],
            'show_in_rest' => true,
        ];

        register_taxonomy('access_tier', ['question'], $args);
    }
}